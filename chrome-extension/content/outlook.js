/**
 * Channel Companion - Outlook Content Script
 * Detects opened emails and injects advisor sidebar for Outlook Web
 */

const API_BASE = 'https://channel-manager-experience-production.up.railway.app/api/live';

class OutlookCompanion {
  constructor() {
  ...
  watchDOMChanges() {
    ...
    // Watch Outlook's main reading pane
    const readingPane = document.querySelector('[role="main"]');
    if (readingPane) {
      observer.observe(readingPane, config);
    } else {
  ...
  detectAndInjectSidebar() {
    // Outlook Web stores email in reading pane
    const emailContent = this.findEmailContent();
    if (!emailContent) return;

    const senderEmail = this.extractSenderEmail(emailContent);
  ...
  findEmailContent() {
    // Outlook reading pane structure
    const readingPane = document.querySelector('[role="article"], [role="document"]');
    return readingPane || document.querySelector('.ae-mailview-content');
  }

  extractSenderEmail(element) {
    // Look for sender info in Outlook headers
    const senderElements = element.querySelectorAll('[title*="@"], .ms-font-l, [aria-label*="From"]');

    for (let elem of senderElements) {
      const email = elem.getAttribute('title') || elem.textContent;
      if (this.isValidEmail(email)) {
        return email.toLowerCase();
      }
    }

    // Alternative: search text content
